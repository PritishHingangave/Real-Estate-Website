...
export const google =async(req,res,next)=>{
    try{
        ...
        if(user)
        {
            ...
            const {password: pass,...rest}=user._doc; // hiding the password
        res
            .cookie('access_token',token,{httpOnly:true})
            .status(200)
            .json(rest);
            ...
            await newUser.save();
            const token=Jwt.sign({id:newUser._id},process.env.JWT_SECRET) //create JWT token
            const {password: pass,...rest}=newUser._doc; // hiding the password
        res
            .cookie('access_token',token,{httpOnly:true})
            .status(200)
            .json(rest);
